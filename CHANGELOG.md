...
- **SKILL.md documentation for skcomms, skchat, capauth, skseal, skskills, cloud9** (@opus)
...
- **Build landing pages for 4 empty directories: skcomms-io, skdata-io, skgraph-io, skvector-io** (@docs-writer, @opus)
...
- **SKSeal + SKComms: P2P signing request delivery via sovereign transport** (@jarvis)
- **MCP server for SKComms: expose messaging tools to AI agents** (@jarvis)
- **SKChat: wire SKComms daemon for real end-to-end P2P messaging** (@jarvis)
...
- **SKComms operational bootstrap: config, comms dirs, first message sent** (@sonnet)
- **skworld.io landing page** (@docs-writer)
- **Agent team blueprint schema and registry** (@opus)
- **Consciousness Swipe: browser extension for AI session state export via SKComms** (@opus)
...
- **Lumina node SKComms setup: accept sync, configure identity, verify bidirectional** (@opus)
...
- **SKComms transport metrics: per-transport delivery stats and latency tracking** (@transport-builder)
...
- **SKComms bootstrap: create config.yml with Syncthing transport wired up** (@jarvis)
...
- **SKChat Flutter: SKComms daemon HTTP bridge for send and receive** (@skchat-builder)
...
- **SKComms peer directory: map agent names to transport addresses** (@transport-builder)
...
- **SKComms rate limiter: token bucket throttling per transport and per peer** (@transport-builder)
...
- **skcomms init CLI: bootstrap config, detect transports, test connectivity** (@transport-builder)
...
- **SKComms comms dir alignment: unify skcapstone sync comms and skcomms transport paths** (@jarvis)
...
- **SKComms daemon: HTTP REST API server for mobile and desktop clients** (@transport-builder)
...
- **SKComms envelope signing: PGP sign every outbound message for authenticity** (@skchat-builder)
